...
export default function GestaoClientes() {
  ...
  const loadUsers = async () => {
    setLoading(true);
    try {
      // Buscar usuários do Supabase - apenas clientes (multimarcas)
      const { data: usersData, error } = await supabase
        .from('users')
        .select(`
          *,
          fornecedores!responsavel_user_id (
            id,
            nome_marca,
            razao_social
          )
        `)
        .eq('role', 'multimarca'); // Mostrar apenas clientes/multimarcas

      if (error) {
        console.error("Erro ao carregar usuários:", error);
        notification.showError("Erro ao carregar usuários do banco de dados.");
        setUsers([]);
      } else {
        setUsers(usersData || []);
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      notification.showError('Falha ao excluir o usuário. Tente novamente.');
    } finally {
                      ...
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-gray-600">
                            {user.fornecedores?.razao_social ||
                             user.fornecedores?.nome_marca ||
                             user.empresa ||
